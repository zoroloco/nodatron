...
function Nodatron(config){
	...
	//Member definitions//
  Nodatron.prototype.processSenderStatus(senderTitle,senderStatus){
    self._log.info(senderTitle+" reported status of:"+senderStatus);
  }

  Nodatron.prototype.removeChildProcess(title){
    var index = self._childProcesses.indexOf(title);
    if(index > -1){
      self._childProcesses.splice(index, 1);
    }
  }

  Nodatron.prototype.processChildMessage(ipMsg){
  ...
  //start up a service defined in the config file.
	Nodatron.prototype.startService = function startService(service){
        ...
        self._log.info("Successfully forked "+service.title+" with pid:"+newProcess.processObject.pid);

        //create a new CPROCESS and add to array
        var newProcess = {
          "processObject" : nodeChildProcess,
          "processTitle"  : service.title,
          "retryFlag"     : service.keepAlive
        }
        self._childProcesses[service.title] = newProcess;
  ...
  Nodatron.prototype.shutdown = function shutdown(INTERRUPT){
    if(!utils.isEmpty(self._childProcesses)){
      ...
      for(var cp in self._childProcesses){
          if(!utils.isEmpty(cp) && !utils.isEmpty(cp.processObject) && cp.processObject.pid != 0){
            //turning off flag so retry won't happen anymore on this child process.
            self._log.warn("Turning off keep-alive flag for "+cp.processObject.title);
            ...
            self._log.warn("Sending "+INTERRUPT+" signal to pid:"+cp.processObject.pid);
            process.kill(cp.processObject.pid,INTERRUPT);
          }//cp
      }//for every child
    }
    else{
      self._log.info("No child processes found to shutdown.");
    }
