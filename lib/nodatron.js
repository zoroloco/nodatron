...
function Nodatron(config){
	...
	process.on('SIGINT', function() {
		self._log.info("Caught interrupt signal(Ctrl-C) - Starting shutdown.");		
		if(!utils.isEmpty(self._childProcesses)){
			for(i = 0; i < self._childProcesses.size(); i++){
				self._childProcesses.kill('SIGTERM');
			}	
		}		
	...
	Nodatron.prototype.startService = function startService(service){
		...
		if(!service.disabled){
			...
			if(!utils.isEmpty(nodeChildProcess)){											
			    self._childProcesses.push(nodeChildProcess);
