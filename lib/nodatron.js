...
function Nodatron(config){
        ...
        if(!utils.isEmpty(ipMsg.senderTitle)){
          ...
          if(!utils.isEmpty(ipMsg.receiverTitle)){
            if(!utils.isEmpty(ipMsg.data)){
              var nodeChild = self.getNodeChild(ipMsg.receiverTitle);//find the child
              if(!utils.isEmpty(nodeChild)){
                nodeChild.send(ipMsg.data);
              }
            }
          }
        }
  ...
  Nodatron.prototype.getNodeChild = function getNodeChild(title){
    if(!utils.isEmpty(self._childProcesses) && self._childProcesses.length>0){
      for(var index=0; index<self._childProcesses.length; index++){
        if(self._childProcesses[index].getTitle() == title){
          return self._childProcesses[index];
        }
      }
    }
  }
  ...
  Nodatron.prototype.removeChildProcess = function removeChildProcess(title){
    if(!utils.isEmpty(self._childProcesses) && self._childProcesses.length>0){
      for(var index=0; index<self._childProcesses.length; index++){
          self._log.info("Removing "+self._childProcesses[index].getTitle());
  ...
  Nodatron.prototype.shutdown = function shutdown(INTERRUPT){
    if(!utils.isEmpty(self._childProcesses) && self._childProcesses.length>0){
      ...
      for(var index=0; index<self._childProcesses.length; index++){
        self._log.info("Shutdown will begin for "+self._childProcesses[index].toString());
          if(self._childProcesses[index].getInstance().pid != 0){
            //send down the kill signal to child
            self._log.warn("Sending "+INTERRUPT+" to "+self._childProcesses[index].toString());
            //process.kill(self._childProcesses[index].getInstance().pid,INTERRUPT);
            self._childProcesses[index].getInstance().kill(INTERRUPT);
          }
      }//for every child
    }
