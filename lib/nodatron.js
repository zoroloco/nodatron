...
function Nodatron(config){
	...
	Nodatron.prototype.startService = function startService(service){
        ...
        //store away the pid
        self._childProcesses[nodeChildProcess.pid] = nodeChildProcess;
        ...
        nodeChildProcess.on('message', (data) => {
          if(!utils.isEmpty(data)){
            ...
            if(!utils.isEmpty(parsedData) && parsedData.length>0){
              var senderPid = parsedData[0];
              self._log.info("Forwarding data to robotSvc");
              //TODO: figure out PID of robotSvc
              //self._childProcesses[].send(data);
            }
            else{
            ...
            self._log.warn("Nodatron received empty message from child process.");
				...
				nodeChildProcess.on('error', function(err){
					...
					self._childProcesses[nodeChildProcess.pid] = '';
					retryService();
				});
