...
function Nodatron(config){
	...
	Nodatron.prototype.startService = function startService(service){
		...
		if(!service.disabled){
			...
			self._log.info("Just attempted to spawn child:"+svcName+" with parameters:\n"+paramArray);

			if(!utils.isEmpty(nodeChildProcess)){
        //store away the pid
        self._childProcesses[service.title] = nodeChildProcess;
        self._log.info("Successfully spawned "+service.title+" with pid:"+nodeChildProcess.pid);
        ...
        nodeChildProcess.on('message', (data) => {
          if(!utils.isEmpty(data)){
            ...
            //figure out the process that sent the message
            //message format is:  pid-message
            var parsedData = data.toString().split("-");
            if(!utils.isEmpty(parsedData) && parsedData.length>0){
              var senderTitle = parsedData[0];
              self._log.info("Nodatron received a message from:"+senderTitle);
              if(senderTitle == 'tcpSvc'){//TODO: don't hard code
                self._childProcesses['robotSvc'].send(data);
              }
            }
            else{
              self._log.error("Error parsing data from child process."+data);
            }
          }
          else{
    ...
    process.on('SIGTERM', function() {//called from /etc/init.d/nodatron.sh from kill pid
      ...
      process.exit();//calls process.on(exit) below
    });
    ...
    process.on('exit', function(){
      self._log.warn("Killing nodatron - Starting shutdown of child processes.");
      self.shutdown();
    })
  ...
  Nodatron.prototype.shutdown = function shutdown(){
