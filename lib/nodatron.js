...
function Nodatron(config){
	...
	process.on('SIGINT', function() {
		self._log.warn("Caught interrupt signal(Ctrl-C) - Starting shutdown.");		
		if(!utils.isEmpty(self._childProcesses)){
			for(var cp_pid in self._childProcesses){	
                if(cp_pid !=0)			
				    process.kill(cp_pid,'SIGINT');
			}	
		}		
	...
	Nodatron.prototype.startService = function startService(service){
		...
		if(!service.disabled){
			...
			if(!utils.isEmpty(nodeChildProcess)){											
			    self._childProcesses.push(nodeChildProcess.pid);
